# -*- coding: utf-8 -*-
"""
Created on Fri Jun  8 17:58:20 2018
...
def GetSearchContent(key):
    ...
    while end_stamp <= end_date:
        ...
        #通过构建URL实现每一天的查询
        url = current_url + '&typeall=1&suball=1&timescope=custom:' + start_stamp.strftime("%Y-%m-%d-%H") + ':' + end_stamp.strftime("%Y-%m-%d-%H") + '&Refer=g'
        driver.get(url)
...
def getContent():
    ...
    for i in range(len(nodes)):
        ...
        try:
            ...
            #WBRZ = nodes[i].find_element_by_xpath("//div[@class='info']/a[@title='微博官方认证']").get_attribute("title")#若没有认证则不存在节点
            WBRZ = nodes[i].find_element_by_xpath(".//div[@class='info']/div/a[contains(@title,'微博')]").get_attribute('title') 
            #print(nodes[8].find_element_by_xpath("//div[@class='info']/a[@title='微博官方认证']").get_attribute("title"))
        except:
            WBRZ = ''
        print('微博认证:', WBRZ)
        dic[i].append(WBRZ)
        ...
        #判断展开全文和网页链接是否存在
        try:
            t = nodes[i].find_element_by_xpath(".//div[@class='content']/p[@class='txt']/a/i[@class='wbicon']").text
            if t == 'c':
                flag = True
            else:
                flag = False
        ...
        try:
            if flag:
                ...
                time.sleep(1)
                WBNR = nodes[i].find_element_by_xpath(".//div[@class='content']/p[2]").text
                print("zhan开了全文")
            else:
            ...
            #WBNR = nodes[i].find_element_by_xpath(".//div[@class='content']/p[@class='txt']").text
            #print(nodes[9].find_element_by_xpath(".//div[@class='content']/p[@class='txt']").text)
        except:
            WBNR = ''
        print('微博内容:', WBNR)
        dic[i].append(WBNR)
...
if __name__ == '__main__':
    ...
    #定义变量
    username = '18363945218'             #输入你的用户名
    password = 'lixuewei123'               #输入你的密码
